...
def build_commands(destination: Path, name: str, solution_format: str, host_version: str | None, mvvm_version: str | None) -> list[Command]:
    ...
    commands = [
        ...
        Command(("dotnet", "new", "classlib", "-n", infrastructure, "-o", f"src/{infrastructure}", "--framework", "net10.0"), root),
        Command(("dotnet", "new", "mstest", "-n", tests, "-o", f"tests/{tests}", "--framework", "net10.0"), root),
    ...
    commands.extend(
        [
            ...
            Command(("dotnet", "build", str(solution), "--no-restore"), root),
            Command(("dotnet", "test", str(solution), "--no-build"), root),
        ]
    )
    return commands
